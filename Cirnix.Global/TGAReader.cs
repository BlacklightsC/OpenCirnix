...
namespace Cirnix.Global
{
    ...
    {
        /// <summary>
        /// TGA 포맷의 바이트 배열을 읽어들여 저장합니다.
        /// </summary>
        /// <param name="buffer">읽어올 바이트 배열입니다.</param>
        /// <param name="filePath">저장될 경로 입니다. 확장자를 제외한 이름을 입력하세요.</param>
        /// <param name="extension">저장될 확장자 입니다.</param>
        /// <returns>변환 성공 여부를 알립니다.</returns>
        public static bool SaveTo(byte[] buffer, string filePath, string extension)
        {
            try
            {
                Bitmap bmp = null;
                ...
                switch(extension.Replace(".",string.Empty).ToUpper())
                {
                    case "PNG":
                    ...
                    case "GIF":
                        ...
                        return true;
                    //case "ICO":
                    //    bmp.Save(filePath + ".ico", System.Drawing.Imaging.ImageFormat.Icon);
                    //    break;
                    default:
                        throw new Exception("Unsupport Extension.");
                }
            }
            catch
            {
